# Exercise and Checklist Library — Transparency, Ethics, and Regulation in Media: The Accountability Layer

Read this during Phase 5 (exercises) and Phase 6 (checklists). Adapt
scenarios to the audience's domain; keep evaluation criteria intact — they
are what make the exercises assessable. In full-module mode, write an
expert-quality model solution for each included exercise.

## Table of Contents
- Part A: Practical Exercises
- Part B: Professional Gate Checklists

---

# Part A: Practical Exercises

## Exercise 1 — Enforce media accountability through transparency documentation (supply-chain findings
**Scenario:** provide a realistic Media Mathematics & Algorithmic Targeting scenario appropriate to the audience (real or closely modeled on documented practice).
**Objective:** enforce media accountability through transparency documentation (supply-chain findings, compensation structures, measurement monopoly, data opacity), governance responses (contract architecture.
**Constraints:** must use the framework taught in the corresponding unit; must state assumptions and trade-offs; one deliverable, length-bounded.
**Deliverable:** the professional artifact this capability produces (memo, table, plan, analysis, or model — as applicable).
**Evaluation criteria:** correct application of the framework; decisions justified with evidence; limitations honestly stated; artifact usable by a colleague without further explanation.

## Exercise 2 — Audit practice
**Scenario:** provide a realistic Media Mathematics & Algorithmic Targeting scenario appropriate to the audience (real or closely modeled on documented practice).
**Objective:** audit practice, disclosure standards, committee structures), the regulatory landscape (competition, privacy integration, political rules, consumer protection, digital-markets law), targeting ethics (manipulation boundaries, vulnerable audiences, attention economy, sustainability), professional standards (code ecosystems, speak-up culture, collective action, individual integrity), and the accountability future (measurement independence, AI transparency, privacy architecture), avoiding rebate concealment, self-audit acceptance, opacity tolerance, vulnerability monetization, sustainability blindness, and integrity flexibility failures.
**Constraints:** must use the framework taught in the corresponding unit; must state assumptions and trade-offs; one deliverable, length-bounded.
**Deliverable:** the professional artifact this capability produces (memo, table, plan, analysis, or model — as applicable).
**Evaluation criteria:** correct application of the framework; decisions justified with evidence; limitations honestly stated; artifact usable by a colleague without further explanation.

## Exercise 3 — Integrated capstone
**Scenario:** one realistic problem spanning the whole section.
**Objective:** take the problem through every unit's framework in sequence.
**Constraints:** all unit artifacts required and internally consistent; evidence discipline maintained throughout.
**Deliverable:** a coherent campaign/portfolio document assembled as real professional documentation.
**Evaluation criteria:** consistency across artifacts; each capability demonstrably exercised; trade-offs stated at each decision point.

---

# Part B: Professional Gate Checklists

Use verbatim as module appendices and as workshop job aids. Every item must
force a decision or produce an artifact — items that are merely
inspirational get cut.

## B1. Enforce media accountability through checklist
- [ ] Enforce media accountability through transparency documentation (supply-chain findings, compensation structures, measurement monopoly, data opacity), governance responses (contract architecture?
- [ ] Decisions and their justification are documented for review by a colleague

## B2. Audit practice, disclosure standards, checklist
- [ ] Audit practice, disclosure standards, committee structures), the regulatory landscape (competition, privacy integration, political rules, consumer protection, digital-markets law), targeting ethics (manipulation boundaries, vulnerable audiences, attention economy, sustainability), professional standards (code ecosystems, speak-up culture, collective action, individual integrity), and the accountability future (measurement independence, AI transparency, privacy architecture), avoiding rebate concealment, self-audit acceptance, opacity tolerance, vulnerability monetization, sustainability blindness, and integrity flexibility failures?
- [ ] Decisions and their justification are documented for review by a colleague

## B3. Delivery quality checklist
- [ ] Every claim sourced, graded, or flagged as contested?
- [ ] All terminology explained on first use?
- [ ] Trade-offs stated wherever recommendations are context-dependent?
- [ ] Material reads as practitioner-written, not generic?
